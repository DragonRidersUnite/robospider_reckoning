  ...
  class << self
    def create(args, x:, y:)
      p = { c_x: x, c_y: y, w: 40, h: 20, lin_vel: 0, ang_vel: 0, th: 0, path: :body,
            body_shift_x: x, body_shift_y: y, body_shift_th: 0, turret_th: 0,
      ...
      args.outputs[:body].sprites << {
        x: 0, y: 0, w: p.w, h: p.h,
        tile_x: 0, tile_y: 0, tile_w: p.w, tile_h: p.h,
      ...
      p.legs = 6.times.map do |i|
        ...
        { c_x: c_x, c_y: c_y, w: 4, h: 4,
          lin_vel: 0, ang_vel: 0, th: 0,
          path: :leg,
          r: dir * 255,
          grounded: i.even?,
          rel_x: rel_x, rel_y: rel_y,
          rel_anchor_x: rel_anchor_x, rel_anchor_y: rel_anchor_y
        }
      end
    ...
    def render_legs args, p, camera
      leg_joints = p.legs.map do |leg|
        x, y = transform(p.body_shift_x - camera.x, p.body_shift_y - camera.y, p.body_shift_th,
                         leg.rel_anchor_x, leg.rel_anchor_y)
        mid_joint_x = (x * 0.3 + (leg.c_x - camera.x) * 0.7)
        mid_joint_y = (y * 0.3 + (leg.c_y - camera.y) * 0.7)
        [
          {
            c_x: x, c_y: y,
            x: x - 2, y: y - 2, w: 4, h: 4, g: 255
          },
          {
            c_x: mid_joint_x, c_y: mid_joint_y,
            x: mid_joint_x - 2, y: mid_joint_y - 2, w: 4, h: 4, b: 255
          },
          leg
        ]
      end

      args.outputs.lines << leg_joints.map do |leg|
        [
          {
            x: leg[0].c_x, y: leg[0].c_y,
            x2: leg[1].c_x, y2: leg[1].c_y,
            r: 221, g: 144, b: 22
          },
          {
            x: leg[1].c_x, y: leg[1].c_y,
            x2: leg[2].c_x - camera.x, y2: leg[2].c_y - camera.y,
            r: 113, g: 50, b: 94
          }
        ]
      end
    ...
    def render args, p, camera
      args.outputs.sprites << [p.merge(x: p.body_shift_x - p.w / 2 - camera.x, y: p.body_shift_y - p.h / 2 - camera.y, angle: p.body_shift_th * 180 / Math::PI)]
      ...
      args.outputs.sprites << {
        x: turret_c_x - 6, y: turret_c_y - 6, w: 31, h: 33 - 21, r: 255,
        angle: turret_th * 180 / Math::PI,
        path: "sprites/body.png",
        tile_x: 0, tile_y: 21, tile_w: 31, tile_h: 33 - 21,
        angle_anchor_x: 0.2, angle_anchor_y: 0.5
      }
