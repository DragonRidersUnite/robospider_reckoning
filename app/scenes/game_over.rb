module Scene
  class << self
    ...
    def tick_game_over(args)
      ...
      labels << label("#{text(:level)}: #{player.level}", x: args.grid.w / 2, y: args.grid.top - 300, size: SIZE_SM, align: ALIGN_CENTER)
      labels << label("#{text(:enemies_destroyed)}: #{args.state.enemies_destroyed}", x: args.grid.w / 2, y: args.grid.top - 360, size: SIZE_SM, align: ALIGN_CENTER)
      ...
      if (player.contemplating-=1) < 0
        labels << label(:restart, x: args.grid.w / 2, y: args.grid.top - 520, align: ALIGN_CENTER, size: SIZE_SM).merge(a: args.tick_count % 155 + 100)
