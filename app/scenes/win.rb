module Scene
  ...
  # Shown after the player wins
    def tick_win(args)
      ...
      labels <<
        label("#{text(:level)}: #{a_s.player.level}", x: args.grid.w / 2, y: args.grid.top - 300, size: SIZE_SM, align: ALIGN_CENTER)
      labels <<
        label(
          "#{text(:enemies_destroyed)}: #{a_s.enemies_destroyed}",
          x: args.grid.w / 2,
          y: args.grid.top - 360,
          size: SIZE_SM,
          align: ALIGN_CENTER
      ...
      if (a_s.player.contemplating -= 1) < 0
        labels <<
          label(:retry, x: args.grid.w / 2, y: args.grid.top - 520, align: ALIGN_CENTER, size: SIZE_SM)
            .merge(a: a_s.tick_count % 155 + 100)
